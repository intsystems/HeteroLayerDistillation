...
def mutual_information(logits_t, logits_s):
...
def distill(out, batch_logits, temp):    
    g = sm(out)
    f = F.log_softmax(batch_logits)    
    return mutual_information(f, g)
...
def param_loss(batch,logits, model,h):
    x,y,batch_logits = batch    
#     lambda1,temp = h
#     lambda2 = 1.0 - lambda1
    logits_t, logits_s = logits
    out = model(x)    
    distillation_loss = distill(out, logits_t, logits_s)
    student_loss = crit(out, y)                
    loss =  h.dot(distillation_loss) + student_loss
    return loss
...
# def mi_base(exp_ver, run_num, epoch_num, start_lambda1, start_temp, filename, tr_load, t_load, validate_every_epoch, class_num=10,  mode='nodistil', logits = '../code/logits_cnn.npy', seed=42, lr0=1.0):
#     np.random.seed(seed)
#     t.manual_seed(seed)
...
def cifar_with_validation_set(exp_ver, run_num, epoch_num, filename, alg_pars, tr_load, t_load, val_load, validate_every_epoch,  class_num=10, lambdas_ = None,  lr0 = 1e-3, lr = 1.0, clip_grad = 10e-3, mode='opt', no_tqdm = False, seed=42, logits = '../code/logits_mi.npy'):
...
#     for m_e, tr_s_epoch in alg_pars:    
    for _ in range(run_num):
        ...
        lambdas = t.nn.Parameter(t.tensor(np.random.uniform(low=0.0, high=1.0, (num_teacher_layers, num_student_layers)), device=device), requires_grad=True)
        #lambda2 = t.nn.Parameter(t.tensor(np.random.uniform(low=0.0, high=1.0), device=device), requires_grad=True)
#             temp = t.nn.Parameter(t.tensor(np.random.uniform(low=0.1, high=10.0), device=device), requires_grad=True)
...
#             temp.data += lambdas[1]
        ...
        student = MI_student(class_num).to(device)
        optim = t.optim.SGD(student.parameters(), lr=lr0) 
        scheduler = t.optim.lr_scheduler.StepLR(optim, step_size=10, gamma=0.5)               
...
#                 if (mode == 'opt' or (mode == 'splines' and mini_e % tr_s_epoch == 0)):                   
                ...
                for i in range(num_teacher_layers):
                    for j in range(num_student_layers):
                        if lambdas[i, j] > 1.0:
                            lambdas[i, j].data*=0.0
                            lambdas[i, j].data+=1.0
                        if lambdas[i, j] < 0.0:
                            lambdas[i, j].data*=0.0
...
#             if mode == 'opt':
            with open('../log/mi_exp'+exp_ver+'_'+filename+'.jsonl', 'a') as out:
                    out.write(json.dumps({'results':internal_results, 'version': exp_ver})+'\n')
#             if mode == 'splines':
